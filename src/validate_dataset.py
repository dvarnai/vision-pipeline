...
# Verify no overlapping samples generated by the split
overlap = set(train.indices) & set(val.indices)
if len(overlap) > 0:
    print(f"Error: Training and validation sets have overlapping samples: {overlap}")
    exit(1)

# Iterate over data to verify images can be read and that they are all grayscale
image_ids = np.array([None]*len(dataset), dtype=object)
for i, id in enumerate(dataset.image_ids):
    image_ids[i] = id
    img = np.array(Image.open(os.path.join(args.images_path, id)))

    # verify all RGB values match at each pixel (is grayscale)
    r, g, b = img[..., 0], img[..., 1], img[..., 2]
    if not np.all((r == g) & (g == b)):
        print(f"Error: Image {id} is not grayscale")
        exit(1)
